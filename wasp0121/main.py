...
def retr_lpos_four(para, gdat, modltype):
    ...
    if ((para < gdat.limtpara[0, :]) | (para > gdat.limtpara[1, :])).any():
        ...
        llik, deptatmo = retr_llik_four(gdat, offs, amfo, shft, modltype)
...
def retr_modl_four(gdat, offs, amfo, shft, modltype):
    
    phasshft = (gdat.meanphas - shft * np.pi / 180.) * 2. * np.pi
    deptatmo = occl - deptnigh
    modl = offs + 0.5 * 1e-6 * deptatmo * np.cos(phasshft)
    for k in range(3):
        modl += 0.5e-6 * amfo[k] * np.sin((k + 1) * gdat.meanphas * 2. * np.pi) 
    for k in range(2):
        modl += 0.5e-6 * amfo[k+5] * np.cos((k + 2) * gdat.meanphas * 2. * np.pi)
    
    modl[gdat.indxoccl] -= occl * 1e-6
    
    return modl, deptatmo


def retr_modl_spec(gdat, tmpt):
    
    specplan = gdat.thpt / meanwlen**5 / (np.exp(gdat.conswlentmpt / meanwlen / tempdayy) - 1.)
...
def retr_llik_spec(gdat, tmpt, modltype):
    
    specplan = retr_modl_spec(gdat, offs, amfo, tempdayy, tempstar, shft, modltype)
    deptseco = specplan / gdat.specstar
...
def retr_lpos_spec(para, gdat):
    ...
    if ((para < gdat.limtpara[0, :]) | (para > gdat.limtpara[1, :])).any():
        ...
        tmpt = para
        specplan = retr_modl_spec(gdat, tmpt)
        wlen = np.linspace(gdat.minmwlen, gdat.maxmwlen, 100)
        spec = np.trapz(specinpt, wlen)
        llik = -0.5 * np.sum((spec - gdat.stdvplan)**2 / gdat.variplanobsd)
        lpos = llik
        
    return lpos


def retr_llik_four(gdat, offs, amfo, shft, modltype):
    
    modl, deptatmo = retr_modl_four(gdat, offs, amfo, shft, modltype)
    llik = -0.5 * np.sum((modl - gdat.data)**2 / gdat.datastdv**2)
...
def init():
    ...
    #meanwlenband = gdat.data[:, 0] * 1e-3
    #thptband = gdat.data[:, 1]
    #
    ...
    for k in gdat.indxdatatmpt:
        ...
        binswlen = np.linspace(gdat.minmwlen, gdat.maxmwlen, 100)
    ...
    axistwin.plot(meanwlenband, thptband, color='grey', ls='--', label='TESS')
    ...
    gdat.thpt = scipy.interpolate.interp1d(meanwlenband, thptband)(meanwlen)
    ...
    # parse the data
    gdat.meanphas = (arrythrd[:, 0] - epoc) / peri
    data = arrythrd[:, 1]
    datastdv = arrythrd[:, 2]
    
    gdat.meanphas = (gdat.meanphas) % 1.
    ...
    for modltype in listmodltype:
        ...
        # amfo
        gdat.limtpara[0, 1:8] = -1e3
        gdat.limtpara[1, 1:8] = 1e3
        # tempdayy
        gdat.limtpara[0, 8] = 0.
        gdat.limtpara[1, 8] = 5000.
        # tempstar
        gdat.limtpara[0, 9] = 0.
        gdat.limtpara[1, 9] = 10000.
        if modltype == 'shft':
            # phas
            gdat.limtpara[0, 10] = -10.
            gdat.limtpara[1, 10] = 10.
        
        gdat.indxoccl = np.where(abs(gdat.meanphas - 0.5) < dura / peri)[0]
        ...
        indxphasfineoccl = np.where(abs(gdat.meanphasfine - 0.5) < dura / peri)[0]
        for k, indxsampplottemp in enumerate(indxsampplot):
            ...
            phasmodlfine[k, :], deptatmofine = retr_modl(gdat.meanphasfine, offs, amfo, tempdayy, tempstar, shft, modltype, indxphasfineoccl)
        
        figr, axis = plt.subplots(figsize=figrsize[0, :])
        axis.errorbar(gdat.meanphas, data, yerr=yerr, color='black', marker='o', ls='', alpha=0.2, markersize=1)
        for k, indxsampplottemp in enumerate(indxsampplot):
